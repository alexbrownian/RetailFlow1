#!/usr/bin/env python
"""
run_overlays.py - build ONLY the price overlays (notebooks 11-16).

Use this to refresh the overlay charts without the full live pipeline. It
assumes the aggregates are already in data/processed (from update_data.py,
a git pull + hydrate, or a manual run of notebooks 08/09/10) and just
refreshes the price overlays.

    python run_overlays.py               # hydrate -> pull prices -> run 11-16
    python run_overlays.py --no-prices   # skip the Bloomberg pull (prices already pulled)
    python run_overlays.py --no-hydrate  # skip the ABSTRACTED_DATA -> data/processed copy
    python run_overlays.py --only 13     # run just one overlay

Every step's output streams live to the terminal.
"""
...
def stream(cmd):
    """Run a command with its output streaming live."""
    print(">>>", " ".join(cmd), flush=True)
    return subprocess.run(cmd, cwd=ROOT).returncode


def main():
    p = argparse.ArgumentParser(description="Run the price-overlay notebooks 11-16.")
    p.add_argument("--no-prices", action="store_true", help="skip pull_bloomberg_prices.py")
    p.add_argument("--no-hydrate", action="store_true", help="skip ABSTRACTED_DATA -> data/processed")
    ...
    if not args.no_hydrate:
        print("[1/3] hydrate ABSTRACTED_DATA -> data/processed", flush=True)
    ...
    for name, needs, made_by in chosen:
        ...
        # "-m nbconvert" imports the module directly - no PATH dependence
    ...
    print("open notebooks 11-16 in Jupyter to view the charts.")
