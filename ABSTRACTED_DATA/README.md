...
GitHub and the internal machine. It holds five small parquet files (~2 MB
...
- **External machine (needs raw text):** notebooks 01, 02, 04, 06, 07 read
  the raw `posts.parquet` (private, gitignored) and write the five
  aggregates above.
- **Internal machine (numbers only):** notebooks 03, 05, 08–16 read only the
  five aggregates — so they run where no raw post ever exists.
...
fetched posts and **merges** them into the five files — counts add, sentiment
