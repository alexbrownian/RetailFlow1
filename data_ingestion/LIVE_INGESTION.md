...
`python api_calls/fetch_all.py` — it checks `.env` first and calls only the
sources whose keys are filled (`--check` previews without calling).

| Credential | Used by | How to get it |
|---|---|---|
| `FETCHLAYER_KEY` | `fetch_reddit_live.py` and `fetch_x_live.py` (preferred backends for both) | fetchlayer.dev dashboard. 1 credit per request; verify with `python api_calls/test_fetchlayer.py` (1 credit) |
