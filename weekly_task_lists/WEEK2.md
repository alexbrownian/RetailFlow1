...
- [ ] **Counting = raw mention counts only** (score-based weighting was
      removed as a look-ahead leak — design_decisions.xlsx #30).
...
## Backtest inputs — the complete shopping list

**Price data (the only thing the repo can't generate):**

| What | Format | Notes |
|---|---|---|
| Daily closes for the anchor ETFs | `data/prices/<TICKER>.csv`, columns `date,close` | Minimum: GLD, MAGS, BTC, SMH. Better: all ~20 anchors in `THEME_ETFS` so every theme is tested in one pass. Unadjusted last price is fine for ETFs |
| `SPY.csv` | same | The buy-and-hold benchmark every result must beat |
| Trading calendar | derived from the price CSVs themselves | No extra source needed — a date in SPY.csv = a trading day |

Remember: MAGS exists only from Apr 2023; BTC trades 7 days/week (align on
calendar days, no weekend roll); a strategy on SMH must also beat *holding*
SMH, not just cash.

**Already generated by the notebooks (no new work):** daily ticker counts
(02), theme counts (04), ticker/theme sentiment (06/07), conviction z (08),
the screening CSV. **Only if you later backtest single names:** delisted
tickers' price histories (Bloomberg has them; the DELISTED_TICKERS
supplement only fixes the *mention* side) — one more reason to start with
theme ETFs. **For the live gate:** stored signal snapshots from the
paper-trading period (computed at decision time, never revised).

## Candidate trading signals (test them AGAINST each other)

| # | Signal | Trades when | What it assumes |
|---|---|---|---|
| S1 | **Theme mention take-off** — trailing rolling-z of raw theme mentions crosses K | attention surges | attention spikes on THEMES skew bullish (direction-blind) |
| S2 | **Theme conviction z** — mentions × net_bullish, z vs trailing history, crosses K | crowd is unusually large AND leaning bullish | the lexicon direction is right more often than not |
| S3 | **Divergence filter** (overlay, not entry) — veto S1/S2 entries while the crowded-top flag is on (attention up, mood deteriorating) | — | deteriorating mood into a crowd = late-stage move |
| S4 | **Cross-sectional rotation** — each week long the top-k themes by cross-sectional z | weekly rebalance | relative attention leadership persists ~a week |

Run S1 and S2 through the identical harness: **if S2 (with sentiment)
doesn't beat S1 (attention only) across both windows, sentiment hasn't
earned its complexity — that comparison IS the Stage-2 go/no-go.**

## When to trade, and how conviction scales

**Timing (fixed, non-negotiable):** signals computed at a fixed UTC cutoff
from data dated ≤ t only, with TRAILING baselines (never whole-window
stats — live-you won't know the future average) → enter at the NEXT open →
hold exactly 5 trading days → exit at open. No discretionary exits in v1;
every exit rule is another parameter to overfit.

**Conviction stacking — count the agreeing signals (0–5):**

| Check | +1 if |
|---|---|
| Attention | trailing mention z > K |
| Direction | 5d sentiment change > 0 (vs own baseline) |
| Combined | conviction z > K |
| No red flag | crowded-top divergence NOT active |
| Cross-source | Reddit AND X both rising (only where X coverage exists) |

Read it as: score 1 = note it, score 2–3 = standard entry, score 4–5 =
high conviction. **But in the v1 backtest, trade FIXED size and just LOG
the score per trade** — then check afterwards whether higher scores
actually earned higher returns. If they did, v2 sizes positions by score
(e.g. half / full / 1.5×, hard cap per theme); if they didn't, the
stacking idea dies cheaply. Sizing rules added before that evidence are
...
tested) plus `notebooks/10_mention_price_lag.ipynb` reading
`data/prices/*.csv` and the signal outputs (06/07 = sentiment, 08 =
conviction composite, **09 = the actual entry log** —
`trade_signals.parquet` with dates, ETFs and conviction scores, ready to
be joined against prices). Ask Claude to build it in the next session —
this file is the spec. Note: `THEME_ETFS` maps EVERY theme to its anchor
instrument, so the backtest can cover more than the original four.
